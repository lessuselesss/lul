              ...
              subvolume = lib.mkMerge (
                map (subvol: {
                  "${subvol}" = {
                    ...
                    target = lib.mkMerge (map (target: "${target.mountPoint}/.snapshots/${subvol}") cfg.btrfsTargets);
                  };
                }) cfg.subvolumes
              );
            };
          ...
          lib.imap0 (
            ...
            in
            {
              "time-machine-rsync-${toString idx}" = {
                ...
                wants = [
                  "${builtins.replaceStrings [ "/" ] [ "-" ] (lib.strings.removePrefix "/" target.mountPoint)}.mount"
                ];
